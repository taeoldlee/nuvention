...
async function main() {
  ...
  // ─── HELPER ───
  const img = (id, w = 800) => `https://images.unsplash.com/photo-${id}?w=${w}&fit=crop&auto=format&q=80`;
  ...
  for (const b of brandData) {
    brands.push(await prisma.brandProfile.create({
      data: {
        ...
        profilePhotoUrl: img(opData[b.idx].avatar, 200),
        budgetMin: b.min,
        budgetMax: b.max,
      },
    }));
  ...
  for (const c of cpData) {
    profiles.push(await prisma.creatorProfile.create({
      data: {
        ...
        vibeTags: c.tags,
        profilePhotoUrl: img(crData[c.idx].avatar, 200),
        tier: c.tier,
      },
    }));
  }
