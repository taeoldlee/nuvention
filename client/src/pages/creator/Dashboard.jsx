      ...
      <div className="mb-8">
        ...
        {subtitle && (
          <p className="font-body text-muted text-sm flex items-center gap-1.5">
            <svg
              className="w-4 h-4 text-creatorAccent"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.5}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {briefs.map((brief) => {
              ...
              const deliverables =
                ...
                [];
              const timeline =
                brief.timeline || brief.request?.timeline || '';
              const matchId = brief.matchId || brief.id || brief._id;

              return (
                <button
                  key={matchId}
                  onClick={() => navigate(`/creator/brief/${matchId}`)}
