...
export default function CreatorStepDone({ uploading, uploadProgress, uploadError }) {
  ...
  return (
    <div className="text-center py-8">
      ...
      <p className="font-body text-muted max-w-md mx-auto mb-4">Welcome to Locale. Here's how it works from here.</p>

      {uploading && (
        <div className="max-w-sm mx-auto mb-6 bg-creatorLight/50 rounded-xl p-4">
          <p className="text-sm font-medium text-creator mb-2">Portfolio uploading in the background... {uploadProgress}%</p>
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div className="bg-creator h-1.5 rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
          </div>
          <p className="text-xs text-muted mt-2">Feel free to explore your dashboard while this finishes.</p>
        </div>
      )}

      {uploadError && (
        <div className="max-w-sm mx-auto mb-6 bg-red-50 border border-red-200 rounded-xl p-4">
          <p className="text-sm text-red-700 font-body">{uploadError}</p>
        </div>
      )}
